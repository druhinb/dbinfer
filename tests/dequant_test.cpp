...
int main() {
  ...
  // scales[0..3] feed the high nibble of sub-blocks 4..7. sc_sb/m_sb are the
  // independently hand-computed unpack results the dequant must reproduce.
  const int q4k_sc[8] = {1, 2, 3, 4, 17, 18, 19, 20};
  const int q4k_m[8] = {5, 6, 7, 8, 20, 21, 22, 23};
  const std::uint8_t q4k_scales[12] = {0x41, 0x42, 0x43, 0x44, 0x45, 0x46,
...

  {
    ...
    std::memcpy(blk + 4, q4k_scales, 12);
    for (int e = 0; e < 256; ++e) {
    ...
    for (int e = 0; e < 256; ++e) {
      ...
      const float want = d * q4k_sc[sb] * static_cast<float>(qn[e]) - dmin * q4k_m[sb];
      char name[32];
...

  {
    ...
    for (int e = 0; e < 256; ++e) {
      const int g = e / 64, w = e % 64;
      const int sb = 2 * g + (w >= 32 ? 1 : 0);
      const float step = d * static_cast<float>(q4k_sc[sb]);
      max_step = std::max(max_step, step);
      const float t = static_cast<float>(3 + (e % 10));
      xs[e] = step * (t + 0.3f) - dmin * static_cast<float>(q4k_m[sb]);
    }
    std::byte blk[144];
    std::memset(blk, 0, sizeof blk);
    const std::uint16_t d_bits = 0x3C00, dmin_bits = 0x3800;
    ...
    for (int e = 0; e < 256; ++e) {
      ...
      const int sb = 2 * g + (w >= 32 ? 1 : 0);
      const float step = d * static_cast<float>(q4k_sc[sb]);
      float qf = std::round((xs[e] + dmin * static_cast<float>(q4k_m[sb])) / step);
      qf = std::clamp(qf, 0.0f, 15.0f);
