...
class Backend {
 public:
  ...
  [[nodiscard]] virtual std::expected<void, Error> quantize_q8_0(const float* x, std::size_t rows,
                                                                 std::size_t in,
                                                                 std::byte* out) = 0;
  ...
  [[nodiscard]] virtual std::expected<void, Error> mul_mat_group(
      WeightType type, std::span<const std::byte* const> weights, std::span<const std::size_t> outs,
      const float* A, std::span<float* const> C, std::size_t in) = 0;
  ...
  [[nodiscard]] virtual std::expected<void, Error> attention(const float* q, const float* k_cache,
  ...
  // layout changes.
  [[nodiscard]] virtual std::expected<void, Error> setup_offload(
      std::span<const LayerQ> layers, float* k_base, float* v_base, std::size_t capacity,
