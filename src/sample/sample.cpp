...
std::expected<void, Error> validate(const SamplerParams &p) {
  if (p.temperature < 0.0f)
    return std::unexpected(Error{"temperature must be >= 0"});
  if (p.top_p < 0.0f || p.top_p > 1.0f)
    return std::unexpected(Error{"top_p must be in [0, 1]"});
  if (p.min_p < 0.0f || p.min_p > 1.0f)
    return std::unexpected(Error{"min_p must be in [0, 1]"});
  if (p.repeat_penalty <= 0.0f)
    return std::unexpected(Error{"repeat_penalty must be > 0"});
  return {};
}
...
void apply_penalties(std::span<Candidate> c, std::span<const std::int32_t> recent,
  ...
  for (std::int32_t id : recent)
    ++counts[id];
  for (const auto &[id, count] : counts) {
    if (id < 0 || static_cast<std::size_t>(id) >= c.size())
      continue;
...
void top_k(std::vector<Candidate> &c, int k) {
  if (k <= 0 || static_cast<std::size_t>(k) >= c.size())
    return;
...
void min_p(std::vector<Candidate> &c, float min_p) {
  ...
  for (const Candidate &x : c)
    max_logit = std::max(max_logit, x.logit);
...
std::int32_t Sampler::sample(const float *logits, std::size_t n,
  ...
  if (params_.temperature == 0.0f && !active)
