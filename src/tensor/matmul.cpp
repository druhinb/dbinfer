...
void matvec_f16(const std::uint16_t* W, const float* x, float* y, std::size_t out, std::size_t in) {
  for (std::size_t o = 0; o < out; ++o) {
    ...
    for (std::size_t i = 0; i < in; ++i) acc = std::fma(f16_to_f32(row[i]), x[i], acc);
    y[o] = acc;
  }
}
...
void matvec_q5_0_scalar(const std::byte* W, const float* x, float* y, std::size_t out,
  ...
  for (std::size_t o = 0; o < out; ++o) {
    ...
    for (std::size_t b = 0; b < nblocks; ++b) {
      ...
      for (std::size_t k = 0; k < kBlockSize; ++k) acc += scratch[k] * xb[k];
    }
    y[o] = acc;
  }
}
...
void matvec_q4_k_scalar(const std::byte* W, const float* x, float* y, std::size_t out,
  ...
  for (std::size_t o = 0; o < out; ++o) {
    ...
    for (std::size_t s = 0; s < nsb; ++s) {
      ...
      for (std::size_t k = 0; k < kSuperBlockSize; ++k) acc += scratch[k] * xb[k];
    }
    y[o] = acc;
  }
}
...
const QuantDispatch& quant_dispatch() {
  static const QuantDispatch d = [] {
    ...
    if (k == "scalar") return QuantDispatch{matvec_q8_0_scalar, matvec_q4_0_scalar};
    if (k == "sdot" && f.dotprod) return QuantDispatch{matvec_q8_0_sdot, matvec_q4_0_sdot};
    if (f.i8mm) return QuantDispatch{matvec_q8_0_i8mm, matvec_q4_0_i8mm};
    if (f.dotprod) return QuantDispatch{matvec_q8_0_sdot, matvec_q4_0_sdot};
...
void matvec_quant_fused(const MatvecJob* jobs, std::size_t njobs, const float* x, std::size_t in) {
  ...
  parallel_for(pool, total, kRowTile, [&](std::size_t rb, std::size_t re) {
    ...
    for (std::size_t j = 0; j < njobs; ++j) {
      ...
      if (lo < hi) matvec_rows(jobs[j].w, x, xq, jobs[j].y, lo - o0, hi - o0, in);
      off = o1;
    }
  });
}
...
void matmul(const float* A, const float* W, float* C, std::size_t m, std::size_t out,
  ...
  for (std::size_t r = 0; r < m; ++r) {
    ...
    for (std::size_t o = 0; o < out; ++o) {
      ...
      for (std::size_t i = 0; i < in; ++i) acc += arow[i] * wrow[i];
      crow[o] = acc;
    }
  }
}
...
void matmul_f16_rows(const std::uint16_t* W, const float* A, float* C, std::size_t m,
  ...
  for (std::size_t o = lo; o < hi; ++o) {
    ...
    for (std::size_t i = 0; i < in; ++i) {
      ...
      for (std::size_t r = 0; r < m; ++r) acc[r] = std::fma(w, A[r * in + i], acc[r]);
    }
    for (std::size_t r = 0; r < m; ++r) C[r * out + o] = acc[r];
  }
}
...
void matmul_accel(const float* A, QuantMatrix w, float* C, std::size_t m, std::size_t out,
  ...
  for (std::size_t o = 0; o < out; ++o) dequant_row(w, o, in, wdense.data() + o * in);
#ifdef DBINFER_HAVE_ACCELERATE
