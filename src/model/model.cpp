...
std::expected<Model, Error> Model::load(const GgufFile& file) {
  ...
  if (auto e = normw("output_norm.weight", dim, m.output_norm_)) return std::move(*e);
  ...
  if (out_w == nullptr) {
    ...
    if (auto e = weight("output.weight", cfg.vocab_size, dim, m.lm_head_)) return std::move(*e);
  }
  ...
  for (std::size_t l = 0; l < cfg.n_layers; ++l) {
    ...
    if (auto e = normw(p + "attn_norm.weight", dim, L.attn_norm)) return std::move(*e);
    if (auto e = weight(p + "attn_q.weight", cfg.n_heads * hd, dim, L.attn_q)) return std::move(*e);
    ...
    if (auto e = weight(p + "attn_output.weight", dim, cfg.n_heads * hd, L.attn_output))
      return std::move(*e);
    if (auto e = optbias(p + "attn_q.bias", cfg.n_heads * hd, L.attn_q_bias)) return std::move(*e);
    ...
    if (auto e = optbias(p + "attn_v.bias", cfg.n_kv_heads * hd, L.attn_v_bias))
      return std::move(*e);
    if (auto e = normw(p + "ffn_norm.weight", dim, L.ffn_norm)) return std::move(*e);
    if (auto e = weight(p + "ffn_gate.weight", ff, dim, L.ffn_gate)) return std::move(*e);
    if (auto e = weight(p + "ffn_up.weight", ff, dim, L.ffn_up)) return std::move(*e);
    if (auto e = weight(p + "ffn_down.weight", dim, ff, L.ffn_down)) return std::move(*e);
  }
