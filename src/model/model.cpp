...
using gguf::detail::as_f32;
using gguf::detail::as_u64;
using tensor::matvec_quant;
using tensor::matvec_quant_fused;
using tensor::parallel_for;
using tensor::rmsnorm;
using tensor::rope;
using tensor::silu;
using tensor::softmax;
using tensor::thread_pool;

namespace {

const TensorInfo* find_tensor(const GgufFile& f, const std::string& name) {
...
std::optional<std::unexpected<Error>> bind_weight(const GgufFile& file, const std::string& name,
                                                  std::size_t out, std::size_t in,
                                                  tensor::QuantMatrix& dst) {
  const TensorInfo* t = find_tensor(file, name);
  if (t == nullptr) return std::unexpected(Error{name + " missing", "", 0});
...
std::optional<std::unexpected<Error>> bind_normw(const GgufFile& file, const std::string& name,
                                                 std::size_t n, const float*& dst) {
  const TensorInfo* t = find_tensor(file, name);
  if (t == nullptr) return std::unexpected(Error{name + " missing", "", 0});
  if (t->type != GgmlType::F32) return std::unexpected(Error{name + " expected F32", "", 0});
  if (t->shape[0] != n)
...
std::optional<std::unexpected<Error>> bind_optbias(const GgufFile& file, const std::string& name,
                                                   std::size_t n, const float*& dst) {
  const TensorInfo* t = find_tensor(file, name);
  if (t == nullptr) {
    dst = nullptr;
...
std::expected<Config, Error> parse_config(const GgufFile& file) {
  ...
  if (arch_mv == nullptr) return fail("general.architecture missing");
  const auto* arch_p = std::get_if<std::string>(&arch_mv->value);
  if (arch_p == nullptr) return fail("general.architecture is not a string");
  ...
  auto req_u64 = [&](const std::string& key,
    ...
    auto v = as_u64(*mv);
    if (!v) return std::unexpected(Error{key + " is not an integer", "", 0});
    dst = *v;
    return std::nullopt;
  };
  auto req_f32 = [&](const std::string& key, float& dst) -> std::optional<std::unexpected<Error>> {
    ...
    if (mv == nullptr) return std::unexpected(Error{key + " missing", "", 0});
    auto v = as_f32(*mv);
    if (!v) return std::unexpected(Error{key + " is not a float", "", 0});
    dst = *v;
    return std::nullopt;
  };
  ...
  if (auto e = req_u64(arch + ".attention.head_count", u)) return std::move(*e);
  cfg.n_heads = u;
  if (auto e = req_u64(arch + ".attention.head_count_kv", u)) return std::move(*e);
  cfg.n_kv_heads = u;
  if (auto e = req_u64(arch + ".feed_forward_length", u)) return std::move(*e);
  cfg.ffn_length = u;
  if (auto e = req_u64(arch + ".context_length", u)) return std::move(*e);
  cfg.context_length = u;
  if (auto e = req_f32(arch + ".rope.freq_base", cfg.rope_theta)) return std::move(*e);
  ...
  if (cfg.n_heads % cfg.n_kv_heads != 0) return fail("head_count not divisible by head_count_kv");
  cfg.head_dim = cfg.embedding_length / cfg.n_heads;
  cfg.gqa_factor = cfg.n_heads / cfg.n_kv_heads;

  std::optional<std::uint64_t> vocab;
  if (const MetaValue* vs = file.find_meta(arch + ".vocab_size")) vocab = as_u64(*vs);
  if (!vocab) {
    if (const MetaValue* toks = file.find_meta("tokenizer.ggml.tokens"))
      if (const auto* a = std::get_if<gguf::MetaArray>(&toks->value)) vocab = a->values.size();
  }
  if (!vocab) return fail("cannot determine vocab size");
...
KVCache::KVCache(std::size_t n_layers, std::size_t max_seq, std::size_t n_kv_heads,
    ...
    : policy_(policy),
      ...
      layer_stride_(capacity_ * n_kv_heads * head_dim),
      pos_stride_(n_kv_heads * head_dim),
      n_kv_heads_(n_kv_heads),
      head_dim_(head_dim),
      n_blocks_((head_dim + kKvBlock - 1) / kKvBlock),
...
void quant_head(const float* src, std::size_t n, std::int8_t* dst, float* scales) {
  ...
  for (std::size_t start = 0; start < n; start += kKvBlock, ++b) {
    ...
    float max_abs = 0.0f;
    for (std::size_t i = start; i < end; ++i) max_abs = std::max(max_abs, std::fabs(src[i]));
    const float scale = max_abs / 127.0f;
    scales[b] = scale;
    const float inv = scale > 0.0f ? 1.0f / scale : 0.0f;
    for (std::size_t i = start; i < end; ++i) {
      const std::int64_t q = std::lround(src[i] * inv);
      dst[i] = static_cast<std::int8_t>(std::clamp<std::int64_t>(q, -127, 127));
    }
  }
}
...
std::size_t KVCache::slot_for(std::size_t pos) const {
  if (pos < policy_.n_sink) return pos;
...
void KVCache::requantize_kgroup(std::size_t layer, std::size_t g, std::size_t within) {
  ...
  for (std::size_t kh = 0; kh < n_kv_heads_; ++kh) {
    ...
    for (std::size_t w = 0; w <= within; ++w) {
      if (first + w < n_sink_) continue;
      const float* raw = raw_base + w * pos_stride_ + kh * head_dim_;
      for (std::size_t c = 0; c < head_dim_; ++c) sc[c] = std::max(sc[c], std::fabs(raw[c]));
    }
    for (std::size_t c = 0; c < head_dim_; ++c) sc[c] /= 127.0f;
    for (std::size_t w = 0; w <= within; ++w) {
      if (first + w < n_sink_) continue;
      const float* raw = raw_base + w * pos_stride_ + kh * head_dim_;
      std::int8_t* dst =
      ...
      for (std::size_t c = 0; c < head_dim_; ++c) {
        ...
        const std::int64_t q = std::lround(raw[c] * inv);
        dst[c] = static_cast<std::int8_t>(std::clamp<std::int64_t>(q, -127, 127));
      }
    }
  }
}
...
std::size_t KVCache::residents(Resident* out) const {
  ...
  std::size_t count = 0;
  for (std::size_t t = 0; t < sinks; ++t) out[count++] = {t, static_cast<std::int32_t>(t)};

  std::size_t window_start = n_sink;
  if (n_seen_ > n_sink && n_seen_ - n_sink > window) window_start = n_seen_ - window;
...
std::expected<void, Error> Model::bind_weights(const GgufFile& file) {
  const std::size_t dim = cfg_.embedding_length;
  const std::size_t ff = cfg_.ffn_length;
  const std::size_t hd = cfg_.head_dim;

  if (auto e = bind_weight(file, "token_embd.weight", cfg_.vocab_size, dim, token_embd_))
    return std::move(*e);

  if (auto e = bind_normw(file, "output_norm.weight", dim, output_norm_)) return std::move(*e);

  const TensorInfo* out_w = find_tensor(file, "output.weight");
  if (out_w == nullptr) {
    cfg_.tied_embeddings = true;
    lm_head_ = token_embd_;
  } else {
    cfg_.tied_embeddings = false;
    if (auto e = bind_weight(file, "output.weight", cfg_.vocab_size, dim, lm_head_))
      return std::move(*e);
  }
  ...
  for (std::size_t l = 0; l < cfg_.n_layers; ++l) {
    ...
    if (auto e = bind_weight(file, p + "attn_q.weight", cfg_.n_heads * hd, dim, L.attn_q))
      return std::move(*e);
    if (auto e = bind_weight(file, p + "attn_k.weight", cfg_.n_kv_heads * hd, dim, L.attn_k))
      return std::move(*e);
    if (auto e = bind_weight(file, p + "attn_v.weight", cfg_.n_kv_heads * hd, dim, L.attn_v))
      return std::move(*e);
    if (auto e = bind_weight(file, p + "attn_output.weight", dim, cfg_.n_heads * hd, L.attn_output))
      return std::move(*e);
    if (auto e = bind_optbias(file, p + "attn_q.bias", cfg_.n_heads * hd, L.attn_q_bias))
      return std::move(*e);
    if (auto e = bind_optbias(file, p + "attn_k.bias", cfg_.n_kv_heads * hd, L.attn_k_bias))
      return std::move(*e);
    ...
    if (auto e = bind_weight(file, p + "ffn_gate.weight", ff, dim, L.ffn_gate))
      return std::move(*e);
    if (auto e = bind_weight(file, p + "ffn_up.weight", ff, dim, L.ffn_up)) return std::move(*e);
    if (auto e = bind_weight(file, p + "ffn_down.weight", dim, ff, L.ffn_down))
      return std::move(*e);
  }
  return {};
}
...
std::expected<Model, Error> Model::load(const GgufFile& file) {
  Config cfg = TRY(parse_config(file));

  Model m;
  m.cfg_ = cfg;
  TRY(m.bind_weights(file));
...
void Model::attend_dense_f32(std::size_t layer, std::int32_t pos, KVCache& kv, float scale,
                             DebugCapture* dbg) {
  const std::size_t hd = cfg_.head_dim;
...
void Model::attend_dense_int8(std::size_t layer, std::int32_t pos, KVCache& kv, float scale,
                              DebugCapture* dbg) {
  const std::size_t hd = cfg_.head_dim;
  const std::size_t nh = cfg_.n_heads;
  const std::size_t gqa = cfg_.gqa_factor;
    ...
    for (std::size_t h = hbegin; h < hend; ++h) {
      ...
      for (std::size_t pp = 0; pp <= last; ++pp) {
        ...
        if (pp < n_sink) {
          ...
          for (std::size_t i = 0; i < hd; ++i) dot += qh[i] * ks[i] * static_cast<float>(kp[i]);
        }
        sc[pp] = dot * scale;
      }
      softmax(sc, sc, last + 1);
...
void Model::attend_ring(std::size_t layer, std::int32_t pos, KVCache& kv, float scale,
                        DebugCapture* dbg) {
  const std::size_t hd = cfg_.head_dim;
  const std::size_t nh = cfg_.n_heads;
  const std::size_t gqa = cfg_.gqa_factor;
  ...
  rope(q_.data(), &q_pos, cfg_.rope_theta, nh, 1, hd);
    ...
    for (std::size_t h = hbegin; h < hend; ++h) {
      ...
      for (std::size_t r = 0; r < n_res; ++r) {
        if (i8) {
          ...
          for (std::size_t i = 0; i < hd; ++i) kbuf[i] = kp[i];
        }
        const std::int32_t rp = res[r].rope_pos;
        rope(kbuf.data(), &rp, cfg_.rope_theta, 1, 1, hd);
        float dot = 0.0f;
...
void Model::decode_layer(std::size_t layer, float* x, std::int32_t pos, KVCache& kv,
  ...
  rmsnorm(x, L.attn_norm, cfg_.rms_eps, normed_.data(), 1, dim);
  // q k v share the normed input, fused under one barrier.
  ...
  if (L.attn_k_bias) {
    for (std::size_t i = 0; i < nkv * hd; ++i) k_[i] += L.attn_k_bias[i];
  }
  if (L.attn_v_bias) {
    for (std::size_t i = 0; i < nkv * hd; ++i) v_[i] += L.attn_v_bias[i];
  }
  ...
  if (!kv.ring()) {
    ...
    rope(q_.data(), &p, cfg_.rope_theta, nh, 1, hd);
    rope(k_.data(), &p, cfg_.rope_theta, nkv, 1, hd);
    kv.append(layer, static_cast<std::size_t>(pos), k_.data(), v_.data());
    if (!kv.int8()) {
      attend_dense_f32(layer, pos, kv, scale, dbg);
    } else {
      attend_dense_int8(layer, pos, kv, scale, dbg);
    }
  } else {
    attend_ring(layer, pos, kv, scale, dbg);
  }

  matvec_quant(L.attn_output, attn_.data(), proj_.data(), dim, nh * hd);
  for (std::size_t i = 0; i < dim; ++i) x[i] += proj_[i];

  const std::size_t ff = cfg_.ffn_length;
  rmsnorm(x, L.ffn_norm, cfg_.rms_eps, normed_.data(), 1, dim);
  // one barrier covers both gate and up over the normed input.
  const tensor::MatvecJob gate_up[] = {
      {L.ffn_gate, ffn_gate_.data(), ff},
      {L.ffn_up, ffn_up_.data(), ff},
  };
  matvec_quant_fused(gate_up, 2, normed_.data(), dim);
  silu(ffn_gate_.data(), ffn_gate_.data(), ff);
  for (std::size_t i = 0; i < ff; ++i) ffn_gate_[i] *= ffn_up_[i];
  matvec_quant(L.ffn_down, ffn_gate_.data(), ffn_down_.data(), dim, ff);
  for (std::size_t i = 0; i < dim; ++i) x[i] += ffn_down_[i];

  if (dbg != nullptr && dbg->layer_out != nullptr) {
    for (std::size_t i = 0; i < dim; ++i) dbg->layer_out[i] = x[i];
  }
}
...
bool Model::layers_f16(std::size_t n) const {
  ...
  for (std::size_t l = 0; l < limit; ++l) {
    ...
    for (gguf::GgmlType ty : t)
      if (ty != GgmlType::F16) return false;
  }
  return true;
}
...
bool Model::layers_offloadable(std::size_t n) const {
  ...
  for (std::size_t l = 0; l < limit; ++l) {
    ...
    const gguf::GgmlType t[] = {L.attn_q.type,   L.attn_k.type, L.attn_v.type,  L.attn_output.type,
                                L.ffn_gate.type, L.ffn_up.type, L.ffn_down.type};
    for (gguf::GgmlType ty : t)
      if (!to_weight_type(ty)) return false;
  }
  return true;
}
...
bool Model::set_gpu_offload(backend::Backend* backend, std::size_t n_layers) {
  ...
  if (backend == nullptr || n_layers == 0) return true;
...
void Model::decode_token_gpu(float* x, std::int32_t pos) {
  ...
  if (!r) {
    for (std::size_t l = 0; l < gpu_layers_; ++l) decode_layer(l, x, pos, kv_, nullptr);
    return;
  }
...
const float* Model::forward(std::int32_t token, std::int32_t pos) {
  ...
  if (offload && gpu_head_) {
    ...
    for (std::size_t l = 0; l < cfg_.n_layers; ++l) decode_layer(l, x_.data(), pos, kv_, nullptr);
  } else {
    if (offload) decode_token_gpu(x_.data(), pos);
...
void Model::decode_layer_chunk(std::size_t layer, float* x, std::int32_t pos0, std::size_t n,
  ...
  for (std::size_t c = 0; c < n; ++c) {
    ...
    if (L.attn_k_bias) {
      for (std::size_t i = 0; i < nkv * hd; ++i) kc[i] += L.attn_k_bias[i];
    }
    if (L.attn_v_bias) {
  ...
  for (std::size_t i = 0; i < n * dim; ++i) x[i] += s.proj[i];
  ...
  for (std::size_t i = 0; i < n * ff; ++i) s.gate[i] *= s.up[i];
  tensor::matmul_quant(L.ffn_down, s.gate.data(), s.down.data(), n, dim, ff);
  for (std::size_t i = 0; i < n * dim; ++i) x[i] += s.down[i];
}
...
void Model::forward_chunk(const std::int32_t* tokens, std::int32_t pos0, std::size_t n,
  ...
  for (std::size_t c = 0; c < n; ++c) embed(tokens[c], s.x.data() + c * dim);
  for (std::size_t l = 0; l < cfg_.n_layers; ++l)
...
// clang-format off
constexpr std::array<std::byte, 8> kKvMagic{
std::byte{'D'}, std::byte{'B'}, std::byte{'K'}, std::byte{'V'},
std::byte{'C'}, std::byte{'A'}, std::byte{'C'}, std::byte{'H'}};
// clang-format on
constexpr std::uint32_t kKvVersion = 1;

template <typename T>
void put_scalar(std::vector<std::byte>& buf, const T& v) {
...
struct ByteReader {
  ...
  template <typename T>
  std::expected<T, Error> scalar() {
    if (pos + sizeof(T) > size) return std::unexpected(Error{"kv prefix cache truncated", "", pos});
    T v;
...
std::expected<std::vector<std::byte>, Error> read_whole_file(const std::string& path) {
  ...
  if (f == nullptr) return std::unexpected(Error{"cannot open kv prefix cache " + path, path, 0});
  ...
  std::fclose(f);
  if (!ok) return std::unexpected(Error{"read error on kv prefix cache " + path, path, 0});
  return data;
}

}  // namespace

std::expected<void, Error> Model::save_kv_prefix(
  ...
  if (prefix_len == 0) return std::unexpected(Error{"kv prefix is empty", path, 0});
  if (kv_.n_seen() < prefix_len)
    return std::unexpected(Error{"cache holds fewer positions than the prefix", path, 0});

  std::vector<std::byte> buf;
  for (std::byte b : kKvMagic) put_scalar(buf, b);
  ...
  for (std::int32_t t : prefix_tokens) put_scalar(buf, t);
  ...
  std::fclose(f);
  if (!ok) return std::unexpected(Error{"write error on kv prefix cache " + path, path, 0});
  return {};
}

std::expected<std::size_t, Error> Model::load_kv_prefix(
  ...
  for (std::byte want : kKvMagic) {
    ...
    if (got != want) return std::unexpected(Error{"kv prefix cache bad magic", path, 0});
  }
